class Click {
    ...
    async start(client, message, args, prefix, MessageEmbed, Database) {
        ...
        if (Database.Cache.get(`Click.${message.channel.id}`) >= 2)
            return message.reply(`${e.Deny} | O limite de Clicks Game é de 2 por canal, ok? Só esperar algum dos ativos finalizar.`)
        ...
        function registerClickGame() {
            Database.Cache.add(`Click.${message.channel.id}`, 1)
        }

        function unregisterClickGame() {
            if (Database.Cache.get(`Click.${message.channel.id}`) <= 0)
                Database.Cache.delete(`Click.${message.channel.id}`)
            else Database.Cache.subtract(`Click.${message.channel.id}`, 1)
        }
