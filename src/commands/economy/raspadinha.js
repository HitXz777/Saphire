...
module.exports = {
    ...
    run: async (client, message, args, prefix, MessageEmbed, Database) => {
        ...
        let raspadinhaData = await Database.Client.findOne({ id: client.user.id }, 'Raspadinhas')

        if (raspadinhaData?.Raspadinhas?.Channels?.includes(message.channel.id))
            return message.reply(`${e.Deny} | Já tem uma raspadinha sendo aberta neste canal.`)
        ...
        const collector = msg.createReactionCollector({
            ...
            .on('collect', (reaction, user) => {
                ...
                addRaspadinha()
                return startRaspadinha(msg)
            })
            .on('end', () => {
                if (control.accepted) return
                deleteChannelRegister()
                return msg.edit({ content: `${e.Deny} | Comando cancelado.` }).catch(() => { })
            })

        async function startRaspadinha(msg) {
            ...
            const collector = msg.createMessageComponentCollector({
                ...
                .on('collect', async interaction => {
                    ...
                    control.clicks++
                    await editButtom(customId, buttonIndex)
                    checkButtons(customId, buttonIndex)
                    return
                })
                .on('end', () => {
        ...
        function editButtom(customId, buttonIndex) {
        ...
        async function registerChannel() {
            await Database.Client.updateOne(
                { id: client.user.id },
                { $push: { ['Raspadinhas.Channels']: message.channel.id } },
                { upsert: true }
            )
        }

        async function deleteChannelRegister() {
            await Database.Client.updateOne(
                { id: client.user.id },
                { $pull: { ['Raspadinhas.Channels']: message.channel.id } }
            )
        }
