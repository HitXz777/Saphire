class Bet {
    ...
    static Moeda = require('../../../../modules/functions/public/moeda')
    static Colors = require('../../../../modules/functions/plugins/colors')
    static passCode = require('../../../../modules/functions/plugins/PassCode')
    static Database = require('../../../../modules/classes/Database')
}

Bet.prototype.execute = async (client, message, args, prefix, MessageEmbed) => {

    let { e, Moeda, Colors, passCode, Database } = Bet
...
Bet.prototype.betWithUser = async (client, message, args, prefix, MessageEmbed, member) => {

    let { e, Moeda, Database } = Bet

    if (member.id === client.user.id)
        return message.reply(`${e.Deny} | Você não pode apostar comigo. Afinal, eu posso manipular os resultados ao meu favor ${e.Nagatoro}`)

    if (member.id === message.author.id)
        return message.reply(`${e.Deny} | Opa! Nada de apostar contra você mesmo.`)
    ...
    if (!memberData)
        return message.reply(`${e.Deny} | Vish... Eu não encontrei ${member.user.tag} no banco de dados...`)

    let memberMoney = memberData.Balance || 0

    if (!memberMoney || memberMoney <= 0)
        return message.reply(`${e.Deny} | ${member.displayName} não tem dinheiro para brincar.`)

    if (value > memberMoney)
        return message.reply(`${e.Deny} | ${member.displayName} não tem todo esse dinheiro.`)
    ...
    let msg = await message.reply({
        content: `${e.QuestionMark} | ${member}, você está sendo desafiado por ${message.author} para um *bet* no valor de **${value} ${moeda}**.\n> *A aposta será realizada no momento em que ${member.displayName} clicar em "aceitar".*`,
        components: buttons
    }), collected = false

    let collector = msg.createMessageComponentCollector({
        filter: int => [member.id, message.author.id].includes(int.user.id),
        time: 60000,
    })
        .on('collect', interaction => {
            ...
            if (customId === 'accept' && user.id !== member.id) return

            Database.subtract(member.id, value)
            collected = true
            msg.delete().catch(() => { })
            return executeBet()
        })
    ...
    async function executeBet() {
        ...
        let winner = [message.author, member.user][Math.floor(Math.random() * 2)]
        let loser = winner.id === member.id ? message.author : member.user
        ...
        message.channel.send(`👑 | ${winner} ganhou a aposta contra ${loser}! E com sua vitória, faturou **${value} ${moeda}**.${taxaValidate}`)
