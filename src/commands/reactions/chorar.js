...
module.exports = {
  ...
  execute: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let rand = g.Chorar[Math.floor(Math.random() * g.Chorar.length)]
    let texto = args.join(" ")
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Chorar[Math.floor(Math.random() * g.Chorar.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
