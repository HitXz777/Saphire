...
module.exports = {
  ...
  run: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let rand = g.Surprise[Math.floor(Math.random() * g.Surprise.length)]
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} :o`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Surprise[Math.floor(Math.random() * g.Surprise.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
