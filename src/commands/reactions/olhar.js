...
module.exports = {
  ...
  run: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} está olhando...`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Olhando[Math.floor(Math.random() * g.Olhando.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
