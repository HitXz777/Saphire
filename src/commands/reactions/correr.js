...
module.exports = {
  ...
  execute: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let rand = g.Correr[Math.floor(Math.random() * g.Correr.length)]
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} está correndo`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Correr[Math.floor(Math.random() * g.Correr.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
