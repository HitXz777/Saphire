...
module.exports = {
  ...
  run: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let rand = g.Fury[Math.floor(Math.random() * g.Fury.length)]
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} está FURIOSO(a)!`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Fury[Math.floor(Math.random() * g.Fury.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
