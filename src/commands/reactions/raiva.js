...
module.exports = {
  ...
  run: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} está de raiva`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Raiva[Math.floor(Math.random() * g.Raiva.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
