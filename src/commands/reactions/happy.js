...
module.exports = {
  ...
  run: async (client, message, args, prefix, MessageEmbed, Database) => {
    ...
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} está feliz`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Happy[Math.floor(Math.random() * g.Happy.length)]); msg.edit({ embeds: [embed] }).catch(() => { }) })
