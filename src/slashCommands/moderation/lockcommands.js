...
module.exports = {
    name: 'lockcommands',
    description: '[moderation] Bloqueie todos os meus comandos em canais específicos',
    dm_permission: false,
    type: 1,
    ...
    options: [
        ...
        {
            name: 'bots',
            description: 'Bloquear comandos de bots?',
            type: 5
        }
    ],
    async execute({ interaction: interaction, database: Database, emojis: e, guildData: guildData }) {
        ...
        if (guildData?.Blockchannels.Channels?.includes(channel.id))
            return await interaction.reply({
                content: `${e.Info} | O canal ${channel} já está bloqueado.`
            })

        const msg = await interaction.reply({
            content: `${e.QuestionMark} | Você deseja bloquear todos os meus comandos no canal ${channel}?`,
            fetchReply: true
        }),
            emojis = ['✅', '❌']
        ...
        return msg.awaitReactions({
            ...
            if (reaction.emoji.name === emojis[0]) {
                ...
                return msg.edit({
                    content: `✅ | ${interaction.user} bloqueou todos os meus comandos no canal ${channel}.`
                }).catch(() => { })
        ...
        async function BloquearBots() {
            ...
            if (guildData?.Blockchannels.Bots?.includes(channel.id))
                return await interaction.reply({
                    content: `${e.Check} | ${channel} já está bloqueado.`
                })

            const msg = await interaction.reply({
                content: `${e.QuestionMark} | ${interaction.user}, você quer bloquear todos os comandos de todos os bots neste canal?\n${e.SaphireObs} | Vale lembrar que Administradores **NÃO** são imunes a esse bloqueio.`,
                fetchReply: true
            }),
                emojis = ['✅', '❌'],
                filter = (reaction, user) => ['✅', '❌'].includes(reaction.emoji.name) && user.id === interaction.user.id
            ...
            return msg.awaitReactions({ filter, max: 1, time: 15000, errors: ['time'] }).then(async collected => {
                ...
                if (reaction.emoji.name === emojis[0]) {
                    ...
                    return msg.edit(`✅ | ${interaction.user} bloqueou todos comandos de todos os bots canal ${channel}.`).catch(() => { })
